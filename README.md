...
Vicon-provided capabilities are generated by [Vicon Tracker 3.9](https://vicon-help.atlassian.net/wiki/spaces/Tracker39/overview) and accessed through [Vicon DataStream SDK 1.12.0](https://vicon-help.atlassian.net/wiki/spaces/DSSDK112/overview).

| Capability | Provider | Python source | MATLAB source |
|---|---|---|---|
| Synchronized frames, frame number and rate, and latency | Vicon | [`ViconTracker.read()`](Python/vicon_tracker.py) | [`viconTracker.read()`](MATLAB/viconTracker.m) |
| Global position, Euler attitude, quaternion, and occlusion | Vicon | [`_read_pose()`](Python/vicon_tracker.py) | [`readPose()`](MATLAB/viconTracker.m) |
...
| Optional serial-to-PPM command output | This repository ([firmware](Arduino/Nano33IoTPpmBridge/Nano33IoTPpmBridge.ino)) | [`send_ppm_channels()`](Arduino/Python/send_ppm_channels.py) | [`sendPPMChannels()`](Arduino/MATLAB/sendPPMChannels.m) |
...
The language guides describe each Vicon return type, occlusion behaviour, motion estimate, and check. The Arduino guide documents the independent command bridge, packet format, upload process, and required bench checks:

- [Python interface and checks](Python/README.md)
- [MATLAB interface and checks](MATLAB/README.md)
- [Arduino command bridge](Arduino/README.md)
...
Users who also need computer-to-transmitter commands can upload the [Nano 33 IoT firmware](Arduino/Nano33IoTPpmBridge/Nano33IoTPpmBridge.ino) and use the matching [Python](Arduino/Python/send_ppm_channels.py) or [MATLAB](Arduino/MATLAB/sendPPMChannels.m) sender. The bridge accepts eight numbered 1000-2000-microsecond channel values and operates independently of Vicon; the calling application decides how tracked states produce commands.

Follow the [Arduino setup and bench guide](Arduino/README.md) before connecting a compatible trainer input. Its configured per-channel fallback values are not universally neutral or safe.
